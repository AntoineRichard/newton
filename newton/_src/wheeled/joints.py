...
"""Joint configuration helpers for wheeled vehicles."""
...
def configure_wheel_axle_joints(
    ...
    *,
    ...
    This helper is intended for analytical tire-model setups where wheel spin is
    represented by tire control inputs instead of physical revolute axle DOFs.
    It must be called before :meth:`ModelBuilder.finalize`. Selected joints are
    converted to :class:`JointType.FIXED`; their revolute coordinate/DOF data is
    removed and their constraint data is replaced with fixed-joint constraints.
