...
- Add `enable_axial_contact_projection` and `enable_plane_cylinder_contact_collapse` options to `CollisionPipeline` and `NarrowPhase`; disable them to recover a distributed, sinkage-dependent plane-cylinder contact footprint for rolling bodies. Defaults preserve existing behavior.
