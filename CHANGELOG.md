...
- Add `newton.vehicles`, a wheeled-vehicle simulation layer: a cohesive `WheeledVehicles` controller wraps a rigid solver (which owns collision and normal support) and owns analytical wheel spin plus a brush combined-slip tire model, with heterogeneous Ackermann/skid-steer/generic drive modes and `vehicle_husky` / `vehicle_rc_car` examples
